...
## Задачи
Архитектурно решение - това, което се изисква, е да се напишат прототипите на функциите и да се опише какво трябва да правят функции и горе-долу как. *БЕЗ* реална имплементация на функциите.

Пример: Напишете функция, която приема масив от положителни числа, въведен от клавиатурата, и го принтира.

Решение:

```
void validateArray(unsigned int** arr, const size_t& rows, const size_t& columns) {      
	// Обхождат се всички елементи по редове:    
		// За всеки елемент се проверява дали той е равен 0.     
		// Ако е равен на 0, тогава се хвърля грешка с подходящ текст, защото 0 не е положително число.      
}

void print(unsigned int** arr, const size_t& rows, const size_t& columns) {      
	// Обхождат се всички елементи по редове:    
		// Всеки елемент се изписва на конзолата.     
		// След като се изпишат елементите от всеки ред, се изписва и '\n', за да може да се отделят елементите.      
}

int main() {
	//Въвеждат се размерите на масива.
